    ...
    public void initializeDefaultCategories() {
        if (categoryRepository.count() == 0) {
            List<Category> defaultCategories = List.of(
                ...
                Category.builder().name("Outros").icon("💸").type(CategoryType.EXPENSE).color("#94A3B8").build(),
                // Receitas
                ...
                Category.builder().name("Outros").icon("💰").type(CategoryType.INCOME).color("#6EE7B7").build()
            );
